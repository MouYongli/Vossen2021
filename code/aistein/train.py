...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.optimizer_type == 'adam':
            ...
            optimizer = torch.optim.SGD(train_params, lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay, nesterov=args.nesterov)
    ...
    def training(self, epoch):
        ...
        print('[Epoch: %d, numImages: %5d]' % (epoch, i * self.args.train_batch_size + image.data.shape[0]))
    ...
    def validation(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            if len(self.val_loader.dataset.class_names) > 2:
                ...
                pred = torch.squeeze(torch.where(pred > 0.5, torch.ones(pred.shape), torch.zeros(pred.shape)) ,dim=1).numpy().astype(np.int)
        ...
        # MIoU = self.evaluator.Mean_Intersection_over_Union(1)
        ...
        self.writer.add_scalar('val/Prec', Prec, epoch)
        self.writer.add_scalar('val/Rec', Rec, epoch)
        ...
        print("Acc:{}, IoU:{}, Precision:{}, Recall: {}".format(Acc, IoU, Prec, Rec))
...
def main():
    ...
    parser.add_argument('--loss-type', type=str, default='bce', choices=['bce', 'ce', 'focal'], help='loss func type (default: bce)')
    ...
    parser.add_argument('--train-batch-size', type=int, default=8, metavar='N', help='input batch size for training (default: auto)')
    ...
    # optimizer params
    parser.add_argument('--optimizer-type', type=str, default='adam', choices=['adam', 'sgd'], help='optimizer type (default: adam)')
    ...
    if args.lr is None:
        ...
        args.lr = lrs[args.dataset.lower()] / (4 * len(args.gpu_ids)) * args.train_batch_size


    if args.checkname is None:
        args.checkname = str(args.model) + "-" + str(args.backbone) + "/" + str(args.train_crop_strategy)
    print(args)
